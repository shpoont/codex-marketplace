...
MARKER = "AGENTS.md self-improvement review"
CONTRACT = "Task contract: plugin-reference/v1"
PLUGIN = "agents-md-self-improvement"
...
def reviewer_for(prompt: str, path: str) -> bool:
    ...
    return ((prompt.startswith(MARKER + "\n") and f"\nProject: {path}\n" in prompt)
...
def main(argv=None) -> int:
    ...
    for command in ("audit", "render"):
        ...
        sub.add_argument("--registry", type=Path, default=root / "agents-md-self-improvement/projects.json")
        if command == "audit":
