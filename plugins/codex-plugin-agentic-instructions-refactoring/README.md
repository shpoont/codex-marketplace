...
codex plugin add codex-plugin-agentic-instructions-refactoring@shpoont-public
```
...
Refresh the catalog, install its current package, then start a fresh task:
...
First public-release candidate with dedicated packaging, consumer instructions, an MIT license and the standard Shpoont release workflow. Both skill instructions and their UI metadata are unchanged from the preceding personal package.
